...
@MsgId(msgId = MsgIdEnum.BS_GS_MatchArenaEnterNormalPveBattle_VALUE)
public class MatchArenaNormalEnterPveBattleHandler extends AbstractHandler<BS_GS_MatchArenaEnterNormalPveBattle> {
    @Override
    protected BS_GS_MatchArenaEnterNormalPveBattle parse(byte[] bytes) throws Exception {
        return BS_GS_MatchArenaEnterNormalPveBattle.parseFrom(bytes);
    }

    @Override
    protected void execute(GameServerTcpChannel gsChn, BS_GS_MatchArenaEnterNormalPveBattle req, int i) {
        ...
        CS_EnterFight enterBattleBuild = CS_EnterFight.newBuilder()
                .setType(BattleSubTypeEnum.BSTE_ArenaMatchNormal)
                .build();
