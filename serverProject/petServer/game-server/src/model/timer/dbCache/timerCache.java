...
public class timerCache extends baseCache<timerCache> implements IbaseCache, Tickable {
    ...
    public boolean init() {
        ...
        addTimer(TimerIdx.TI_LOG_DAILY_SETTLE, TimeUtil.getNextDayStamp(0), TimeUtil.MS_IN_A_DAY,
                TimerTargetType.TT_UPDATE_LOG_DAILY_DATA, TimerExpireType.ET_EXPIRE_BY_TIME, -1, true);

        addTimer(currentTime, ServerConfig.getInstance().getGamePlayUpdateCycle(), TimerTargetType.TT_UPDATE_GAME_PLAY,
                TimerExpireType.ET_EXPIRE_BY_TIME, -1, false);
     ...
     * @return
     * @see TimerIdx
     * @see model.timer.TimerConst.TimerTargetType
     * @see TimerExpireType
     * @see TimerExpireType
     */
    public void addTimer(String idx, long startTime, long cycleInterval, int targetType, int expireType,
        ...
        if (entity != null) {
            LogUtil.warn("timerCache.addTimer,specify idx is already exist, idx =" + idx);
     ...
     * @return -1 未找到指定计时器
     */
    public long getTimerNextTriggerTime(String timeIdx) {
        timerEntity entity = getByIdx(timeIdx);
        if (entity == null) {
            return -1;
        }
