...
public class JedisUtil extends RedisUtils {
     ...
     * 单次更新最大条数
     */
    public static final int EACH_UPDATE_MAX_COUNT = 50;
     ...
     * @return
     */
    public static boolean tryLockWithOutRetry(String lockKey, String lockParam) {
        return tryLockWithRetry(lockKey, lockParam, DEFAULT_LOCK_EXPIRED_TIME, 1);
    }
     ...
     * @return
     */
    public static boolean tryLockWithOutRetry(String lockKey, String lockParam, long lockMs) {
        return tryLockWithRetry(lockKey, lockParam, lockMs, 1);
    }
     ...
     * @return
     */
    public static boolean tryLockWithRetry(String lockKey, String lockParam) {
        return tryLockWithRetry(lockKey, lockParam, DEFAULT_LOCK_EXPIRED_TIME);
    }
     ...
     * @return
     */
    public static boolean tryLockWithRetry(String lockKey, String lockParam, long lockMs) {
        return tryLockWithRetry(lockKey, lockParam, lockMs, DEFAULT_TRY_LOCK_TIMES);
    }
    ...
    public static boolean tryLockWithRetry(String lockKey, String lockParam, long lockMs, int tryTimes) {
        ...
        for (int i = 0; i < tryTimes; i++) {
            try {
                ...
                setParams.px(lockMs);
                setParams.nx();
                String result = jedis.set(lockKey, lockParam, setParams);
                if (OK.equals(result)) {
                    return true;
                }
    ...
    public static <T> T executeFunction(Supplier<T> supplier) {
        if (supplier == null) {
            LogUtil.warn("util.JedisUtil.exec, error params, function is null");
            return null;
        }
     ...
     * @return
     */
    public static <T> T syncUnRetrySupplier(String lockKey, Supplier<T> supplier) {
        return syncExecSupplier(lockKey, DEFAULT_LOCK_EXPIRED_TIME, 1, supplier);
    }
     ...
     * @return
     */
    public static boolean syncUnRetryBooleanSupplier(String lockKey, BooleanSupplier booleanSupplier) {
        return syncExecBooleanSupplier(lockKey, DEFAULT_LOCK_EXPIRED_TIME, 1, booleanSupplier);
    }
