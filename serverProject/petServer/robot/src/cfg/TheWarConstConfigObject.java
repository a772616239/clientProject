...
public class TheWarConstConfigObject implements baseConfigObject {
    ...
    private Map<Integer, Integer> fightstarrecoverenergy;
    ...
    public void setFightstarrecoverenergy(int[][] fightstarrecoverenergy) {

        if (fightstarrecoverenergy == null || fightstarrecoverenergy.length <= 0) {
            return;
        }
        this.fightstarrecoverenergy = new HashMap<>();
        for (int i = 0; i < fightstarrecoverenergy.length; i++) {
            if (fightstarrecoverenergy[i].length < 2) {
                continue;
            }
            this.fightstarrecoverenergy.put(fightstarrecoverenergy[i][0], fightstarrecoverenergy[i][1]);
        }

    }

    public Map<Integer, Integer> getFightstarrecoverenergy() {

        return this.fightstarrecoverenergy;

    }

    public int getRecoverEnergyByFightStar(int fightStar) {
        if (fightstarrecoverenergy == null) {
            return 0;
        }
        return fightstarrecoverenergy.containsKey(fightStar) ? fightstarrecoverenergy.get(fightStar) : 0;
    }
