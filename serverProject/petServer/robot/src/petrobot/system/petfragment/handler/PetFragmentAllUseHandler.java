 ...
 * @date 2019/12/17
 */
@MsgId(msgId = MsgIdEnum.SC_PetFragmentAllUse_VALUE)
public class PetFragmentAllUseHandler extends AbstractHandler<SC_PetFragmentUse> {
    @Override
    ...
    protected void execute(GameServerTcpChannel gsChn, SC_PetFragmentUse result, int i) {
        ...
        if (result.getResult().getRetCode() != RetCodeEnum.RCE_Success) {
            LogUtil.error("petFragmentAllUse failed by " + result.getResult().getRetCode());
        }
